...
`src/utils/imprimirJasper.js` prints a filled report straight to a named Windows printer (`JRPrintServiceExporter` + `PrinterName`), reusing the same `jasperPrint` the PDF is exported from — so paper and archived PDF can't drift. Enabled per-emission via `datos.impresora`, which only the on-prem Starsoft flow sets (from `IMPRESORA_TICKETS`), and only for person-triggered nominadas: the innominadas cron emits in batches minutes later and must never print. Printing is wrapped in its own try/catch inside `generarPdf` on purpose — by then the invoice is already emitted and signed, so a paper jam must not fail the request and send the outbox lock back to `PENDIENTE`, which would double-emit the sale.

...
const { CAMPOS_NOTA_CREDITO, CAMPOS_CONSULTA_LOTE_NOTA_CREDITO } = require('../services/notaDeCreditoService');
...
routes.post(
    ...
    query('fields').optional().isString().bail().custom(validarFields(CAMPOS_CONSULTA_LOTE_NOTA_CREDITO)),
    ...
    notaDeCreditoController.consultarNotasDeCreditoPorIdExternoLote
);
...
routes.post(
    '/reintentar-sifen',
    authJwt(['ADMIN']),
    // `notaCredito` acepta dos formatos: el número impreso completo "EEE-PPP-NNNNNNN" (ej. "001-002-0000062"),
    // que ya lleva la caja embebida, o el secuencial entero acompañado de `caja` (3 dígitos) por separado.
    body('notaCredito').custom((value, { req }) => {
        ...
        throw new Error('Enviá "notaCredito" como "EEE-PPP-NNNNNNN" (ej. "001-002-0000062") o como número entero junto con "caja" de 3 dígitos');
    }),

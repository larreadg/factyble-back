...
const generarQr = async (url, filename) => {
  return new Promise((resolve, reject) => {
    QRCode.toFile(path.resolve(__dirname, '../../public', 'qr', filename), url, (err) => {
      if (err) reject(err);
...
const generarPdf = async (datos) => {
  ...
  const outputPath = path.resolve(__dirname, '../../public', `${datos.facturaId}.pdf`);

  // Crear un HashMap para los parámetros
  const HashMap = java.import("java.util.HashMap");
  const params = new HashMap();
  params.putSync('empresaLogo', datos.empresaLogo);
  ...
  return {
    outputPath: `/public/${datos.facturaId}.pdf`
  }
